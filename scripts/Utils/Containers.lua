...
--------------------------------------------------------------------------
--  History:
--  - 16/ 5/2006    Created by Matthew Jack
--
--------------------------------------------------------------------------
-- Reload with: #Script.ReloadScript("Scripts/Utils/Containers.lua");

-- ** General container **
--
-- Syntax:
--
-- Rough syntax is Container_type.action(instance, key, value)
-- Note that instance:action(key,value) cannot work for performance reasons
-- The API is loosely based on both the STL and Sano container libraries
...
-- Use:
--
-- The contents of containers should _not_ be manipulated directly.
-- As with any such code, the internal implementation should not be assumed.
-- I would prefer to enforce this with metatables to help debugging.
-- Having said that, functionality is currently limited and should be expanded.
--
-- (To be) expanded docs:
-- http://server4.intern.crytek.de/twiki/bin/view/Games/LuaContainers


-- ** Set container **
--
-- Simple unique associative container close to native tables
-- Most similar to std::set and Sano.HashSet
--
-- Properties:
--
-- Stores key/value pairs
-- If just interested in existence (a true set) can ignore values
-- There can only be one entry for each key
-- The number of entries can be queried efficiently
-- The entries can easily be iterated over

System.Log("GMTasks: Begin loading...");

-- Preserve any existing tables

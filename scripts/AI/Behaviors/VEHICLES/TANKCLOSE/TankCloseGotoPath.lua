...
AIBehaviour.TankCloseGotoPath = {
	...
	ACT_DUMMY = function( self, entity, sender, data )
		...
		self:TANKCLOSE_GOTOPATH(  entity, self, data  );
	...
	------------------------------------------------------------------------------------------
	TANKCLOSE_GOTOPATH = function( self, entity, sender, data )
		...
		if (not entity:IsUsingPipe("tankclose_gotopath")) then
	...
	TANKCLOSE_CHECK_POS = function( self, entity )
		...
		local distToTarget = 0.0;
		
		if ( entity.AI.vFollowTarget ~= nil ) then
			distToTarget = DistanceVectors( entity.AI.vFollowTarget, entity:GetPos() );
		end

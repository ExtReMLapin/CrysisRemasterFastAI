...
AIBehaviour.HeliReinforcement = {
	...
	ACT_DUMMY = function( self, entity, sender, data )
		AI.CreateGoalPipe("heli_reinf");
		entity:SelectPipe(1,"heli_reinf",nil,data.iValue);
		self:HELI_LANDING_FOR_REINFORCEMNT( entity, sender, sender, data );
	end,
	...
	---------------------------------------------
	HELI_LANDING_FOR_REINFORCEMNT = function( self, entity, sender, data )
	...
	---------------------------------------------
	HELI_LANDING_CANCELORNOT =  function( self, entity, sender, data )
	...
	---------------------------------------------
	HELI_LANDING_FOR_REINFORCEMNT_SUB = function( self, entity, sender, data )
		...
		if ( entity.AI.bLock == false ) then
			...
			if ( length1 > 1.0 ) then
				...
				if ( length1 > 29.0 ) then
					...
					FastScaleVector( vDirXY, vDirXY, length1 / 4.0  );
				end
				if ( entity.AI.isVtol == true ) then
					...
					if ( length2 > 10.0 ) then
						...
						if (vDirZ.z>-2.0) then		-- if we are not close in the xy plane, then we need to ensure we have some lift to move
							vDirXY.z=2.0;
						elseif (vDirZ.z>-4.0) then
							vDirXY.z=1.5;
						else
							vDirXY.z =  0.0;
						end

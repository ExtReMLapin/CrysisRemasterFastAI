...
local   fHELI_HOVERATTACK2_EMERGENCYSTOP_FROM_VSCLOAK				= 22;
...
AIBehaviour.HeliHoverAttack2 = {
	...
	Constructor = function ( self, entity, sender, data )
		...
		entity.AI.stopCloakShootTime = nil;
	...
	HELI_HOVERATTACK2_GETPARTSDAMAGE = function ( self, entity, sender, data )
		...
		if ( AIBehaviour.HELIDEFAULT:heliCheckDamage( entity, nil, data ) == false ) then
			return;
		end
...
--		System.Log("dt "..dt);
		...
		if ( target and AIBehaviour.HELIDEFAULT:heliCheckHostile( entity, target )==true ) then
			...
			if ( AI.GetTargetType( entity.id ) == AITARGET_MEMORY ) then
				local bCloakedTarget = false;
				if( target.actor ) then
					bCloakedTarget = true;
					if ( entity.AI.CurrentHook ~= fHELI_HOVERATTACK2_VSCLOAK and entity.AI.CurrentHook ~= fHELI_HOVERATTACK2_EMERGENCYSTOP_FROM_VSCLOAK ) then
						AIBehaviour.HeliHoverAttack2:HELI_HOVERATTACK2_VSCLOAK_START( entity );
					end
				end
				if ( entity.AI.bBlockSignal == false  ) then

					if ( System.GetCurrTime() - entity.AI.memorySec > 7.0 and bCloakedTarget == false ) then
		...
		if ( entity.AI.bBlockSignal == false ) then
			...
			elseif ( entity.AI.CurrentHook == fHELI_HOVERATTACK2_EMERGENCYSTOP_FROM_VSCLOAK ) then
				AIBehaviour.HeliHoverAttack2:HELI_HOVERATTACK2_DEBUGLABEL( entity:GetPos(), "HELI_HOVERATTACK2_EMERGENCYSTOP_FROM_VSCLOAK" );
				AIBehaviour.HeliHoverAttack2:HELI_HOVERATTACK2_EMERGENCYSTOP( entity );
...
--		System.DrawLabel( vVec, 2, label, 1, 1, 1, 1);
	...
	HELI_HOVERATTACK2 = function( self, entity )
		...
		if ( target and AIBehaviour.HELIDEFAULT:heliCheckHostile( entity, target )==true ) then
			...
			local vTargetPos = target:GetPos();
			...
			local vDist = {};
			SubVectors( vDist, vTargetPos, vMyPos );
			vDist.z =0;

			SubVectors( vFwd, vTargetPos, vMyPos );
			NormalizeVector( vFwd );
			...
			NormalizeVector( vWng );
			FastScaleVector( vWng, vWng, 30.0 );
			FastSumVectors( vWng, vWng, vMyPos );
	
			SubVectors( vFwd, vWng , vTargetPos  );
			NormalizeVector( vFwd );
			FastScaleVector( vFwd, vFwd, 30.0 );
			FastSumVectors( vFwd, vFwd, vTargetPos );
			SubVectors( vFwd, vFwd, vMyPos );
			NormalizeVector( vFwd );
			...
			vFwd.z = AIBehaviour.HELIDEFAULT:heliUpdateMinZFast( entity, vMyPos.z, height, 2.0 );
			vFwd.z = AIBehaviour.HELIDEFAULT:heliStickToMinZFast( entity, vMyPos.z, vFwd.z );
...
--		AIBehaviour.HELIDEFAULT:heliStickToMinZ( entity, vHover );
	...
	HELI_HOVERATTACK2_PATHATTACK = function( self, entity )
		...
		if ( target and AIBehaviour.HELIDEFAULT:heliCheckHostile( entity, target )==true ) then
			...
			local vTargetPos = target:GetPos();
			local vEntityPos = entity:GetPos();

			local vDist_x, vDist_y, vDist_z;
			vDist_x = vTargetPos.x - vEntityPos.x;
			vDist_y = vTargetPos.y - vEntityPos.y;
			vDist_z = 0;

			local vFwd_x, vFwd_y, vFwd_z;
			vFwd_x, vFwd_y, vFwd_z = NormalizeCompVector( vDist_x, vDist_y, vDist_z );

			local vWng = entity:GetDirectionVector(0);
			local vEntDir1 = entity:GetDirectionVector(1);

			CopyVector( vVel, vEntDir1 );
			vVel.z =0;
			NormalizeVector( vVel );
			...
			local dot = vFwd_x * vWng.x + vFwd_y * vWng.y + vFwd_z * vWng.z;
			local dot2 = vFwd_x * vVel.x + vFwd_y * vVel.y + vFwd_z * vVel.z;
			local dot3 = dotproduct3d( vEntDir1, vActualVel );
			...
			if ( math.abs( dot ) >  math.cos( 60.0 * 3.1416 / 180.0  ) ) then
				if ( LengthCompVector( vDist_x, vDist_y, vDist_z ) < 50.0 ) then
					if ( random(1,256) < 80 ) then
						AI.Signal(SIGNALFILTER_SENDER,1,"HELI_HOVERATTACK2_ADVANCE_START", entity.id);
					else
						AI.Signal(SIGNALFILTER_SENDER,1,"HELI_HOVERATTACK2_GOAWAY_START", entity.id);
			...
			vVelRot.z = AIBehaviour.HELIDEFAULT:heliUpdateMinZFast( entity, vEntityPos.z, height, 3.0 );
			vVelRot.z = AIBehaviour.HELIDEFAULT:heliStickToMinZFast( entity, vEntityPos.z, vVelRot.z );

			AI.SetForcedNavigationCompVec( entity.id, vVelRot.x, vVelRot.y, vVelRot.z );
	...
	HELI_HOVERATTACK2_GOAWAY = function( self, entity )
		...
		if ( target and AIBehaviour.HELIDEFAULT:heliCheckHostile( entity, target )==true ) then
			...
			local vDist_x, vDist_y, vDist_z;
			local vTmp_x, vTmp_y, vTmp_z;
			local vVel_x, vVel_y, vVel_z;

			vDist_x = entity.AI.vTargetRsv.x - vEntityPos.x;
			vDist_y = entity.AI.vTargetRsv.y - vEntityPos.y;
			vDist_z = 0;
			local length = LengthCompVector( vDist_x, vDist_y, vDist_z );
			vDist_x = vDist_x / length;
			vDist_y = vDist_y / length;

			vTmp_x, vTmp_y, vTmp_z = NormalizeCompVector(entity.AI.vTargetRsv.x - entity.AI.vMyPosRsv.x, entity.AI.vTargetRsv.y - entity.AI.vMyPosRsv.y, 0);
			vTmp_x = vTmp_x * 2.0;
			vTmp_y = vTmp_y * 2.0;

			local entDir1 = entity:GetDirectionVector(1);
			vVel_x, vVel_y, vVel_z = NormalizeCompVector( entDir1.x + vTmp_x, entDir1.y + vTmp_y, 0);

			local dotEnd = vVel_x * vDist_x + vVel_y * vDist_y + vVel_z * vDist_z;
			...
			vTmp_x = vEntityPos.x - entity.AI.vMyPosRsv.x;
			vTmp_y = vEntityPos.y - entity.AI.vMyPosRsv.y;
			vTmp_z = 0;
			
			if ( LengthCompVector( vTmp_x, vTmp_y, vTmp_z ) > 250.0 ) then
				AI.Signal(SIGNALFILTER_SENDER,1,"HELI_HOVERATTACK2_ADVANCE_START", entity.id);
				return;
			end
			...
			vVel_x, vVel_y, vVel_z = FastScaleCompVector( vVel_x, vVel_y, vVel_z, cof );

			local height = AIBehaviour.HELIDEFAULT:heliSetForcedNavigationCompVec( entity, vVel_x, vVel_y, vVel_z, 10.0, 3.5 );
			if ( height < 0 ) then
				self:HELI_HOVERATTACK2_EMERGENCYSTOP_START( entity );
				return false;
			end
			...
			vVel_z = AIBehaviour.HELIDEFAULT:heliUpdateMinZFast(entity, vEntityPos.z, height, 3.0 );
			vVel_z = AIBehaviour.HELIDEFAULT:heliStickToMinZFast( entity, vEntityPos.z, vVel_z );

			AI.SetForcedNavigationCompVec( entity.id, vVel_x, vVel_y, vVel_z );
	...
	HELI_HOVERATTACK2_VSCLOAK_START = function( self, entity )
		...
		entity.AI.stopCloakShootTime = System.GetCurrTime() + 1.4;		-- Delay the stop shoot request a little while
	...
	HELI_HOVERATTACK2_VSCLOAK = function( self, entity )
		...
		if ( entity.AI.stopCloakShootTime ~= nil and System.GetCurrTime() > entity.AI.stopCloakShootTime ) then
			AIBehaviour.HELIDEFAULT:heliRequest2ndGunnerStopShoot( entity );
			entity.AI.stopCloakShootTime = nil;
		end
		...
		if ( height < 0  ) then
			self:HELI_HOVERATTACK2_EMERGENCYSTOP_START( entity );
			if ( entity.AI.CurrentHook == fHELI_HOVERATTACK2_EMERGENCYSTOP ) then
				entity.AI.CurrentHook = fHELI_HOVERATTACK2_EMERGENCYSTOP_FROM_VSCLOAK;
			end
			return false;
		end

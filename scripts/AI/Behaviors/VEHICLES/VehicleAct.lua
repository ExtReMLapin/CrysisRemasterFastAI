...
AIBehaviour.VehicleAct = {
	...
	--	entity:Act_Move( data );
		...
		if ( data and data.point ) then
			...
			g_StringTemp1 = "action_gotova"..data.fValue.."x"..data.point2.x;
	...
	ACT_SHOOTAT = function( self, entity, sender, data )
		...
		if ( AI.GetTypeOf( entity.id ) == AIOBJECT_VEHICLE and AI.GetSubTypeOf( entity.id ) == AIOBJECT_HELICOPTER ) then
			...
			AI.CreateGoalPipe("action_v1_shoot_at");
			AI.PushGoal("action_v1_shoot_at", "locate",0,"refpoint");
			AI.PushGoal("action_v1_shoot_at", "lookat",0,0,0,true,1);
			AI.PushGoal("action_v1_shoot_at", "+timeout",1,3.0);
			AI.PushGoal("action_v1_shoot_at", "signal",0,1,"ACT_SHOOTAT_HELI",SIGNALFILTER_SENDER);
			AI.PushGoal("action_v1_shoot_at", "locate",0,"refpoint");
			AI.PushGoal("action_v1_shoot_at", "+firecmd",0,FIREMODE_CONTINUOUS,AILASTOPRES_USE);
			AI.PushGoal("action_v1_shoot_at", "+timeout",1,3.0);
			AI.PushGoal("action_v1_shoot_at", "firecmd",0,0);
			AI.SetRefPointPosition( entity.id, data.point );
			entity:InsertSubpipe( AIGOALPIPE_SAMEPRIORITY, "action_v1_shoot_at", nil, data.iValue );
		else
			-- for tank/AAA/warrior/
			AI.CreateGoalPipe("action_v2_shoot_at");
			AI.PushGoal("action_v2_shoot_at", "locate",0,"refpoint");
			AI.PushGoal("action_v2_shoot_at", "+firecmd",0,FIREMODE_CONTINUOUS,AILASTOPRES_USE);
			AI.PushGoal("action_v2_shoot_at", "+timeout",1,data.fValue);
			AI.PushGoal("action_v2_shoot_at", "firecmd",0,0);
			AI.SetRefPointPosition( entity.id, data.point );
			entity:InsertSubpipe( AIGOALPIPE_SAMEPRIORITY, "action_v2_shoot_at", nil, data.iValue );

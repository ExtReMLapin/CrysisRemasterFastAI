...
---------------------------------------------------------------------------------------------------------------------------------------
	FORCEDISABLEGROUNDALIGN_TRUE = function( self, entity, sender )
		entity.actor:SetForceDisableGroundAlign( true );
	end,
	---------------------------------------------------------------------------------------------------------------------------------------
	FORCEDISABLEGROUNDALIGN_FALSE = function( self, entity, sender )
		entity.actor:SetForceDisableGroundAlign( false );
	end,
	---------------------------------------------------------------------------------------------------------------------------------------
...
--	OnActionDone = function( self, entity, sender, data )
...
--	end,
	...
	ACT_GOTO = function( self, entity, sender, data )
		if ( data and data.point ) then
			...
			g_StringTemp1 = "action_goto"..data.fValue.."x"..data.point2.x;
			AI.CreateGoalPipe(g_StringTemp1);
...
--	---------------------------------------------------------------------
	...
	SYS_FLASHLIGHT_ON = function (self, entity, sender)
		...
		if (item and item.weapon) then
			item.weapon:ActivateLamLight(true);
		end
	end,
	...
	SYS_FLASHLIGHT_OFF = function (self, entity, sender)
		...
		if (item and item.weapon) then
			item.weapon:ActivateLamLight(false);
		end
	end,
	...
	---------------------------------------------------------------------
	AVOID_OBSTACLES_TRUE = function (self, entity, sender)
		AI.SetAvoidObstacles(entity.id, true);
	end,
	
	---------------------------------------------------------------------
	AVOID_OBSTACLES_FALSE = function (self, entity, sender)
		AI.SetAvoidObstacles(entity.id, false);
	end,

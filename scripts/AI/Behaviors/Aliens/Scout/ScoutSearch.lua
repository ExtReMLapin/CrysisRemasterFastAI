...
AIBehaviour.ScoutSearch = {
	...
	SC_FIND_INTERESTING_SPOT = function( self, entity )
		...
		if( entity.AI.searchHoverCount == 0 ) then
			...
			if( anchorName ) then
				local pos, dir = AI.GetAIObjectPositionAndDirectionVector( anchorName );
				if( pos and dir ) then
					-- If found interesting anchor, goto to the anchor.				
					CopyVector( entity.AI.searchLocation, pos );
					CopyVector( entity.AI.searchLocationDir, dir );
	...
	SC_SNIFF_PLAYER = function( self, entity )
		...
		if( targetName ) then
			...
			validPos = AI_G02.GetAlienApproachParams( entity.id, 4, targetPos, targetDir, attackPos, attackDir );	-- 0 = attack pos, 1 = recoil pos

...
--		self:SetAttachmentEffect(0, "weapon_effect", "Alien_Weapons.Freeze_Beam.Hunter_Firemode1", g_Vectors.v000, g_Vectors.v010, 1, 0, 0);
	...
	ACT_GOTO = function( self, entity, sender, data )
		if ( data and data.point ) then
			...
			g_StringTemp1 = "action_gototd"..data.fValue.."x"..data.point2.x;
...
--		local t = AI_G02.CanJumpToPoint(entity.id,r,90,10,0,velocity);
--		if(t) then 
...
--		if(Trooper_IsJumping(entity)) then 
		if(entity.actor:IsFlying() or AI.GetGroupCount(entity.id,GROUP_ENABLED,AIOBJECT_ACTOR)<1) then 
...
--		entity:SelectPipe(0,"do_it_standing");	
	...
	---------------------------------------------------------------------
	MELEE_ONGOING = function(self,entity,sender,data)
...
--			if(AI.GetGroupCount( entity.id, GROUP_ENABLED, AIOBJECT_ACTOR ) ==1) then 

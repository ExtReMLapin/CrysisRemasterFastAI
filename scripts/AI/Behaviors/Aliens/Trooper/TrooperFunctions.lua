...
function Trooper_Jump(trooper,targetPos,startAnim,useEvent,addAngle,bUseSpecialAnim,maxAngle)
	...
	local t = AI_G02.CanJumpToPoint(trooper.id,targetPos,angle,20, flags,velocity);
...
Trooper_CanRetreat = function(entity)
	...
	AI.Signal(SIGNALFILTER_LEADER, 10, "SetDistanceToTarget", entity.id,g_SignalData);
	local count = 0;
	local n = AI.GetGroupCount(entity.id,GROUP_ENABLED,AIOBJECT_ACTOR);
	for i=1,n do
		local member = AI.GetGroupMember(entity.id,i,GROUP_ENABLED,AIOBJECT_ACTOR);
...
Trooper_CanFireMoar = function(entity)
	...
	if(AI.GetGroupCount(entity.id,GROUP_ENABLED,AIOBJECT_ACTOR) <= 2 or
...
--			local height = (hDist-minHdist)/(maxHdist - minHdist)*1.5 + vDist + 3;
			...
			local t = AI_G02.CanJumpToPoint(entity.id,targetPos,90,20,AI_JUMP_CHECK_COLLISION,velocity);
			if(t and t>0.1) then 
				...
				local t1 = AI_G02.CanJumpToPoint(entity.id,jumpPos,1,20,AI_JUMP_ON_GROUND + AI_JUMP_CHECK_COLLISION,entityAI.jumpVel,nil, targetPos);
...
--					angle = angle+20+hDist/2;
			...
			local t = AI_G02.CanJumpToPoint(entity.id,startPos,angle,20,AI_JUMP_CHECK_COLLISION,velocity);
...
--				t = AI_G02.CanJumpToPoint(entity.id,jumpPos,angle2,20,AI_JUMP_CHECK_COLLISION+AI_JUMP_ON_GROUND,velocity);
			end
...
--				AI.SetRefPointPosition(entity.id,jumpPos);
				...
				local t1 = AI_G02.CanJumpToPoint(entity.id,jumpPos,angle2,20,AI_JUMP_ON_GROUND + AI_JUMP_CHECK_COLLISION,entityAI.jumpVel,target.id, startPos);
...
--				return false;
--			end
--			local t = AI_G02.CanJumpToPoint(entity.id,targetPos,angle,20,AI_JUMP_ON_GROUND + AI_JUMP_CHECK_COLLISION,velocity);
--			if(t) then 
...
function Trooper_PerformSecondMeleeJump(entity)
	...
	if(target) then 
		...
		t = AI_G02.CanJumpToPoint(entity.id,targetPos,1,20,AI_JUMP_ON_GROUND + AI_JUMP_CHECK_COLLISION,vel,target);
	end				
...
function TrJump(angle,inAir)
	...
	if(trooper and target) then 
		...
		local t = AI_G02.CanJumpToPoint(trooper.id,targetPos,angle,88, filter,velocity);
		if(t) then 

...
--		end
	...
	OnExposedToExplosion = function( self, entity, sender, data)
		self:OnCloseCollision(entity, sender, data);
	end,
...
--		System.Log(">>"..entity:GetName().." OnGroupMemberMutilated");
	...
	OnTargetCloaked = function(self, entity, sender, data)
		-- The naming is misleading: the system event is sent even if the cloaked person is not the AI-s current target!!!
		
		-- If the AI can see the nano guy go cloak, the range to him is non-zero (See CAIPlayer::HandleCloaking)!!!
		if (data.fValue > 0) then		
			AI.NotifyGroupTacticState(entity.id, 0, GN_NOTIFY_UNAVAIL);
			
			AI.SetRefPointPosition(entity.id, data.point);
			
			Log(entity:GetName().." TARGET CLOAKED");
			
			-- A special goalpipe created for this case:
			--	Note the position where the cloaking has happened, 
			--	otherwise the AI will shoot in random directions depending on its last target!!!
			entity:SelectPipe(0,"sn_target_disappear_reaction");
		end
	end,
	...
	---------------------------------------------
	OnFallAndPlayWakeUp	= function( self, entity, sender, data)

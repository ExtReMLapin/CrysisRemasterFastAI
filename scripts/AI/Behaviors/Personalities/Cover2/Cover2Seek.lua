			...
			if (AI.GetGroupTacticState(entity.id, 0, GE_DEFEND_POS) == 1) then
				...
				elseif(AI_Utils:IsTargetOutsideTerritory(entity) == 0 and AI.GetGroupTacticState(entity.id, 0, GE_NEAREST_SEEK) == 1) then
					...
					if (signal ~= 0 and (targetDist == nil or targetDist >= armWavingDistance)) then
						...
						if (attentionTarget and attentionTarget.AI and attentionTarget.AI.NanoSuitMode == BasicAI.SuitMode.SUIT_CLOAK) then
							-- target cloaked
							entity:SelectPipe(0,"cv_seek_target_cloaked");
						else
							-- not cloaked

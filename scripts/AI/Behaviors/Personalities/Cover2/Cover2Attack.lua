		...
		elseif (state == GS_SEARCH or state == GS_ALERTED or state == GS_IDLE) then
			...
			if (not tacPoint) then
				...
				if (AI.GetGroupTacticState(entity.id, 0, GE_MOST_LOST_UNIT) == 1) then
					...
					local armWavingDistance = 15.0;
					local targetDist = AI.GetAttentionTargetDistance(entity.id);
					
					if (signal ~= 0 and (targetDist == nil or targetDist >= armWavingDistance)) then
						if (signal == -1) then
...
--		entity:SelectPipe(0,"sn_close_combat");
	...
	OnOutOfAmmo = function (self,entity, sender)
		...
		if (targetDist) then
			local distToCheck = 8;
			local currentWeapon = entity:CheckCurWeapon();
			-- entity:CheckCurWeapon returns nil for none, 0 for primary, 1 for secondary
			if(targetDist < distToCheck and currentWeapon == 0 and entity:HasSecondaryWeapon() == 1) then
				entity:SelectSecondaryWeapon();
				return;
			elseif(targetDist > distToCheck and currentWeapon == 1) then
				entity:SelectPrimaryWeapon();
				return;
			end
		end

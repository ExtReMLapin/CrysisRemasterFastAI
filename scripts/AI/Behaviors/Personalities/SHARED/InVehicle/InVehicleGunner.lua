...
AIBehaviour.InVehicleGunner = {
	...
	Constructor = function(self, entity)
		...
		if ( entity.actor:IsPlayer() ) then
			...
			entity.AI.oldAimTurnSpeed = AI.GetParameter(entity.id, AIPARAM_AIM_TURNSPEED);
			entity.AI.oldFireTurnSpeed = AI.GetParameter(entity.id, AIPARAM_FIRE_TURNSPEED);
		...
		AI.ChangeParameter(entity.id,AIPARAM_SIGHTENVSCALE_NORMAL,math.max(0.7,entity.Properties.Perception.sightEnvScaleNormal));
		AI.ChangeParameter(entity.id,AIPARAM_SIGHTENVSCALE_ALARMED,math.max(1.0,entity.Properties.Perception.sightEnvScaleAlarmed));
		
		--AI.Signal(SIGNALFILTER_SENDER,1,"controll_vehicleGunner",entity.id);
    ...
    AI.ChangeParameter(entity.id,AIPARAM_SIGHTENVSCALE_NORMAL,entity.Properties.Perception.sightEnvScaleNormal);
    AI.ChangeParameter(entity.id,AIPARAM_SIGHTENVSCALE_ALARMED,entity.Properties.Perception.sightEnvScaleAlarmed);
...
--		Previous();
	...
	INVEHICLEGUNNER_CHECKGETOFF = function( self, entity )
		...
		if ( entity.AI.bInvehicleBehaviorMode == true ) then
			...
			-- Hack - If the player is standing on the AI's head for 1-2 seconds then get out to avoid looking daft
			if ( g_localActor and DistanceSqVectors2d(entity:GetPos(), g_localActor:GetPos()) < 0.5*0.5 and AI.Hostile(entity.id, g_localActorId, false)) then
				if ( entity.AI.exitCounterClose >= 1 ) then
				  -- EXIT_VEHICLE_STAND would fail as there's an enemy around still
	  			  self:EXIT_VEHICLE_DONE( entity );
				  entity.AI.exitCounterClose = 0;
				  return;
				end
				entity.AI.exitCounterClose = entity.AI.exitCounterClose + 1;
			else
				entity.AI.exitCounterClose = 0;
			end

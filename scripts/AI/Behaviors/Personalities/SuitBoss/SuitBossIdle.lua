...
AIBehaviour.SuitBossIdle = {
	...
	Constructor = function (self, entity)
		...
		AIBehaviour.SuitIdle:Constructor(entity);

		local  difficulty = System.GetCVar("g_difficultyLevel");

		local  hurrMeleeBaseScale = 1.1;	-- easy

		if (difficulty == 2) then
			hurrMeleeBaseScale = 1.2;				-- normal

		elseif (difficulty == 3) then
			hurrMeleeBaseScale = 1.3;				-- hard

		elseif (difficulty == 4) then
			hurrMeleeBaseScale = 1.4;				-- delta

		end

		entity.actor:SetWeaponMeleeBaseScale("Hurricane", hurrMeleeBaseScale);

		entity.actor:SetNanoSuitHealthRegenRateScale(0.0);
		entity.actor:SetFlinchFactor(0.1);
	end,

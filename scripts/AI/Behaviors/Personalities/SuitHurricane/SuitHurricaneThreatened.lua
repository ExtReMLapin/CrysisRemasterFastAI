...
AIBehaviour.SuitHurricaneThreatened = {
	...
	---------------------------------------------
	SUIT_THREATENED = function (self, entity, sender)
		...
		if(target == AITARGET_NONE) then
			...
			fDist2DSq = DistanceSqVectors2d(attPos, entity:GetPos());
		else
			AI.GetAttentionTargetPosition(entity.id, attPos);
			fDist2DSq = DistanceSqVectors2d(attPos, entity:GetPos());
			
			if( target ~= AITARGET_SOUND and fDist2DSq < 3600 ) then
		...
		if ( fDist2DSq > 3600 ) then
			entity:SelectPipe(0,"su_takecover");
		else

...
Player = {
	...
	ammoCapacity =
	{
		...
		tagbullet=10,
		gaussbullet=5*6,
		hurricanebullet=500*3,
		incendiarybullet=30*13,
		shotgunshell=8*8,
		avexplosive=3,
		c4explosive=6,
		claymoreexplosive=3,
...
function Player.Server:OnPostInitClient( channelId )
	--for i,v in ipairs(self.inventory) do
		--self.Client.PickUpItem(self, v, false);
	--end
	
	--if (self.inventory:GetCurrentItemId()) then
		--self.Client.SetCurrentItem(self, self.inventory:GetCurrentItemId());
	--end
end
...
--	HUD:Reset();
	...
	if AI then AI.ResetParameters(self.id, false, self.Properties, self.PropertiesInstance, nil,self.melee) end;
...
function Player:StartThrusterSounds(afterburn)
	...
	self:SetTimer(THRUSTER_TIMER, 100);
end
...
--		self:UseEntity( self.OnUseEntityId, self.OnUseSlot, activation == "press" or activation == "hold");
		if (activation == "press" or activation == "release") then
...
function Player.Client:OnTimer(timerId,mSec)
	...
	elseif (timerId == THRUSTER_TIMER) then
		local frameTime = mSec * 0.001;
...
function Player:SetMotionFxAmount(amount, speed)
	...
	if (self.blurType == 2) then
		...
		if (not speed) then
			...
			local headDir;
			if (self.actor) then
				headDir = self.actor:GetHeadDir(g_Vectors.temp_v3);
			else
				headDir = self:GetDirectionVector(); -- TODO: The call to this function might expect the result as a parameter instead of return value.
			end
...
function Player:UpdateDraw()	
	...
	elseif (self ~= g_localActor or stats.thirdPerson or stats.isOnLadder) then
		...
		local ghostPit = self.actor and self.actor:IsGhostPit();	  
		local isZeroG  = self.actor:IsZeroG();
		...
		elseif ((stats.firstPersonBody or 0)>0 and not ghostPit) then
			...
			local item = self.inventory:GetCurrentItem();
			if (not item or not item.item:IsMounted() ) then
				self:HideAttachment(0, "lower_body", isZeroG, isZeroG);
			end
		else
			self:DrawSlot(0,0);
		end

...
{	
	...
	gameParams =
	{
		...
		boneIDs = 
		{
			BONE_BIP01 = "Scout_Root",
			BONE_SPINE = "Bip01 Neck",
			BONE_SPINE2 = "Bip01 Neck",
			BONE_SPINE3 = "Bip01 Neck",
			BONE_HEAD = "Bip01 Head",
			BONE_EYE_R = "Bip01 Head",
			BONE_EYE_L = "Bip01 Head",
			BONE_WEAPON = "weapon_bone",
			BONE_FOOT_R = "Scout_Root",
			BONE_FOOT_L = "Scout_Root",
			BONE_ARM_R = "Scout_Root",
			BONE_ARM_L = "Scout_Root",
			BONE_CALF_R = "Scout_Root",
			BONE_CALF_L = "Scout_Root"
		},
...
--	pos.z = pos.z + headDir.z*radius*0.75;
	...
	if (entities) then
		...
		for i,entity in ipairs(entities) do
			...
			if( self.Properties.esFaction and entity.Properties.esFaction ) then
				if( self.Properties.esFaction == entity.Properties.esFaction ) then
					sameSpecies = true;
				end
			end
	        ...
	        NegVector(hit.normal);		
					...
					--g_gameRules.Server.RequestMeleeHit(g_gameRules, hit.shooter.id, hit.shooter.id, hit.target.id, 0, hit.pos);
    ...
    if (self.Properties.Vulnerability.dmgRatio < 1) then 
	    ...
	    if (newRatio > props.dmgRatio and prevRatio <= props.dmgRatio) then	      
	      ...
	      if (matEff and matEff.decal and hit.normal) then    
	        Particle.CreateMatDecal(hit.pos, hit.normal, 1, 300, matEff.decal, nil, nil, self.id, hit.partId);       
	      end	            

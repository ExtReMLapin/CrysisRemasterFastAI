...
function BasicActor:CacheResources()
	...
	self:PreLoadParticleEffect( "bullet.hit_flesh.armor" );
	...
	for i = 1, bloodSplatGroundCount do
		Game.CacheResource("BasicActor.lua", self.bloodSplatGround[i], eGameCacheResourceType_Material, 0);
	end
end
...
function BasicActor:ResetAmmoCapacity()
	local g_difficultyLevel = System.GetCVar("g_difficultyLevel");
	
	local difficultyString = "difficulty"..g_difficultyLevel;
	
	if (not System.IsEditor()) then	
		if (self.inventory and self.ammoCapacity) then
			for difficulty,ammoCap in pairs(self.ammoCapacity) do
				if (difficulty == difficultyString) then
					for ammo,capacity in pairs(ammoCap) do
...
function BasicActor:ResetCommon()
	...
	self:KillTimer(ACTOREFFECTS_TIMER);
  ...
  if (doFrost and frozenAmount < 1) then
	...
	if (doFrost) then
		if (self.actorStats.isFrozen) then
		  ...
		  -- Fix for DT #3103: sometimes when thawing out after being frozen speed multiplier would only get back to 0.935 not all the way to 1,
		  -- meaning (among other things) the player couldn't sprint. So let's only slow the player down if the frozen amount > 0.5. [TF]
		  if (frozenAmount > 0.5) then
			  self._frost_params.speedMultiplier = 1- ((frozenAmount - 0.5) * 2.0);
		  else
...
function BasicActor:DoBloodPool()
	...
	if (hits > 0 and splat) then
		...
		Particle.CreateMatDecal(splat.pos, splat.normal, s, 300, self.bloodSplatGround[i], math.random()*360, vecNormalize(dir), splat.entity and splat.entity.id, splat.renderNode, 6, 0, true);
...
function BasicActor.Server:OnHit(hit)	
  ...
  --Log("BasicActor.Server:OnHit (%s)", self:GetName());
	...
	if (self:IsOnVehicle() and hit.type~="heal") then
		...
		local hitType = hit.type or ""
		local hitTypeId = g_gameRules.game:GetHitTypeId(hitType)
		local newDamage = vehicle.vehicle:ProcessPassengerDamage(self.id, self.actor:GetHealth(), hit.damage, hitTypeId, hit.explosion or false);
...
function BasicActor:KnockedOutByDoor(hit,mass,vel)
	if(self==g_localActor or self.Properties.esFaction=="Players" or self:IsDead())then return;end;
...
function BasicActor.Client:OnHit(hit)
	...
	if (string.find(hit.type,"bullet")) then
		...
		if (g_localActor) then
			...
			local particleDirection = ScaleVector(hit.dir, -1.0);
	...
	if (hit.type=="melee") then
		self.lastMelee = 1;
		self.lastMeleeImpulse = hit.damage * 4;
...
function BasicActor:SetActorModel(isClient)
	...
	if (self.currModel ~= model) then
		...
		self:CreateBoneAttachment(0, "alt_weapon_bone01", "left_object_attachment");
...
--		end
	...
	elseif (event == "stanceChanged") then
		--to save some performance, the effects timer for AI will be different.
		if (self.actor:IsPlayer()) then
			self:SetTimer(ACTOREFFECTS_TIMER, 50);
		else
			self:SetTimer(ACTOREFFECTS_TIMER, 250);
		end
	end
end
...
----------------------------------------------------------------------------------------------------
function BasicActor:CanShatter()
	return (not self.actor:IsPlayer());
end

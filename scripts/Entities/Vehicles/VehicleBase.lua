...
--		self.Server.RequestEnter(self, user.id, seatId);
  elseif (idx <= 400) then
...
function VehicleBase:GetSeat(userId)
	for i,s in pairs(self.Seats) do
		local id = s.seat:GetPassengerId();
		if (id == userId) then
			return s;
		end
	end
...
function VehicleBase:GetSeatId(userId)	
	for i,s in pairs(self.Seats) do
		if (s.seat:GetPassengerId() == userId) then
			return i;
		end
	end
...
function VehicleBase.Server:OnHit(hit)
  ...
  local direction = hit.dir;
	...
	self.vehicle:OnHit(targetId, hit.shooterId, hit.damage, hit.pos, hit.radius, hitTypeId, explosion);
	...
	if (AI and hit.type ~= "collision") then
		...
		if (hit.shooter and (AI.GetReactionOf(self.id, hit.shooterId) == Hostile)) then
...
--System.Log(">>> vehicleSoundTimer setting NOW >>>>>>");	
		self.vehicle:StartAISoundTimer(self.AISoundRadius);	
	end
	...
	if (passenger.ai ) then
		...
		if( self.hidesUser == 1 )then
			...
			isOutsideGunner = (seat.seat:GetWeaponCount() > 0) and (seat.seat:GetSoundInOut() == 1);

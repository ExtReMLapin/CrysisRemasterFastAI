...
function DestroyableObject:Explode()
	...
	if( CryAction.IsServer() and NumberToBool(self.Properties.bExplode)) then
		...
		local explo_pos=pos;
		local explo_dir={x=0,y=0,z=0};
		CopyVector(explo_dir, expl.Direction);
		g_gameRules:CreateExplosion(self.shooterId,self.id,expl.Damage,explo_pos,explo_dir,expl.Radius,nil,expl.Pressure,expl.HoleSize,expl.Effect,expl.EffectScale, expl.MinRadius, expl.MinPhysRadius, expl.PhysRadius);
	end
	...
	if(explosion.DelayEffect.bHasDelayEffect==1)then
		if(not self.FXSlot or self.FXSlot==(-1))then
			if(not EmptyString(explosion.DelayEffect.Effect))then
					...
					local rots={x=0,y=0,z=0};
					CopyVector(offs, explosion.DelayEffect.vOffset);
					CopyVector(rots, explosion.DelayEffect.vRotation);
					self:SetSlotPos(self.FXSlot,offs);
					self:SetSlotAngles(self.FXSlot,rots);
			end;
		end;
	end;
end
...
------------------------------------------------------------
function DestroyableObject:OnNanoToolActivated()
	self.Properties.Vulnerability.bNanoRope = 1;
end

------------------------------------------------------------
function DestroyableObject:OnNanoToolDeactivated()
	self.Properties.Vulnerability.bNanoRope = 0;
end
...
function DestroyableObject.Server:OnHit(hit)
	...
	elseif (hit.type=="collision") then 
		...
		if(hit.collision_type=="player_sprinting") then
			damage = damage * mult.CollisionType.fPlayerSprinting;
		elseif(hit.collision_type=="player_sliding") then
			damage = damage * mult.CollisionType.fPlayerSliding;
		else 
	...
	if(hit.type == "collision" and vul.bNanoRope ~= 0) then
		pass = 1;
		damage = damage * 20.0;	
	end
	...
	if (pass) then
		...
		if(explosion.DelayEffect.bHasDelayEffect==1)then
			if(not self.FXSlot or self.FXSlot==(-1))then
				...
				self.Client.ClActivateDelayEffect(self)
			end
		end

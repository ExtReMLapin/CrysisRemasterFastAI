...
g_HunterGlowMax = 15; -- this must be changed to 2/4, according to Tiago
g_HunterBaseGlow = 8.0;  -- dito, percentual
...
Hunter_x = 
{
	...
	gameParams =
	{
		...
		boneIDs = 
		{
			...
			BONE_SPINE = "root",
			BONE_SPINE2 = "root",
			BONE_SPINE3 = "face_connector",
			BONE_HEAD = "face_bigass_gun",
			BONE_EYE_R = "face_bigass_gun",
			BONE_EYE_L = "face_bigass_gun",
			BONE_WEAPON = "face_bigass_gun",
			BONE_FOOT_R = "footBoneFrontRight02",
			BONE_FOOT_L = "footBoneFrontLeft02",
			BONE_ARM_R = "R_humerus",
			BONE_ARM_L = "L_humerus",
			BONE_CALF_R = "R_femur",
			BONE_CALF_L = "L_femur"
		},
...
--				"melee_attack_02",
	...
	IKLimbs = 
	{
		{0,"frontRightTentacle","frontLegRight01","","frontLegRight12",IKLIMB_RIGHTHAND},
		{0,"frontLeftTentacle","frontLegLeft01","","frontLegLeft12",IKLIMB_LEFTHAND},
		{0,"backRightTentacle","backLegRight01","","backLegRight13",0},
		{0,"backLeftTentacle","backLegLeft01","","backLegLeft13",0},
		
		{0,"mouthRightTentacle","frontTentRight01","","frontTentRight16",4},
...
function Hunter_x.Client:OnUpdate(frameTime)
	...
	if(self.actor:GetHealth()>0) then 
		...
		if(_time-self.LastSoundTime > 1) then
			AI.SoundEvent(self:GetWorldPos(g_Vectors.temp), 500, AISOUND_GENERIC, self.id);
			self.LastSoundTime = _time;
		end	
	end	
	--FIXME:VS2 hack
	if (self.deadbodyTouchGround) then
		...
		if (self.deadbodyTouchGround<0.0) then
			g_gameRules:ClientViewShake(self:GetWorldPos(),nil,0,90,60,0.75,0.1);
			self.deadbodyTouchGround = nil;
		end
	end	
...
function Hunter_x:ProcessDetachableSpots(hit)
	...
	if (detachable and not detachable.detached) then
		...
		if (ratio >= detachable.ratio) then
	    	...
	    	g_gameRules:ClientViewShake(hit.pos,nil,0,90,0.1,0.9,0.1);
...
function Hunter_x.Client:OnTimer(timerId,mSec)
  ...
  elseif (timerId == HUNTER_DEATH_EXPLOSION_TIMER) then
    ...
    --g_gameRules:ClientViewShake(stepPos,nil,0,70,3.5,0.15,0.095);    
...
function Hunter_x.Client:OnCollision(hit)
	...
	if (vel>10.0) then
		...
		if (not self.firstCrash) then
			...
			g_gameRules:ClientViewShake(pos,nil,0,90,80,0.9,0.1);
		else
			Particle.SpawnEffect("explosions.ScoutCrashB.a", hit.pos, g_Vectors.v001,randomF(0.5,1.5));
				
			g_gameRules:ClientViewShake(pos,nil,0,30,40,0.25,0.09);
		end
...
function Hunter_x:ScriptEvent(event,value,str)
	...
	elseif (event == "footstep") then
		...
		g_gameRules:ClientViewShake(stepPos,nil,0,70,0.3,0.05,0.095);
...
function Hunter_x:SetGrabbingScheme(scheme)
	if (scheme and scheme == "Left") then
		...
		self.grabParams.boneGrabOffset = { x=0, y=0, z=-1.0 };

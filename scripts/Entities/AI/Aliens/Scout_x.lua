...
function Scout_x.Server:OnHit(hit)--(shooterId, weaponId, matName, damage, hitType, pos, normal, partId, radius)
	...
	if(shooter~=nil) then
		...
		if (self.Properties.esFaction ~= shooter.Properties.esFaction) then
...
function Scout_x:AutoDestruct()
	...
	if ( self.iAutoDestructTimer == 2 ) then
		...
		g_gameRules:ClientViewShake(pos,nil,0,75,2,0.25,0.0075);
		g_gameRules:CreateExplosion(self.id,self.id,self.PropertiesInstance.explosionDamage,pos,nil,10);
...
--		  Log("fallback: ScoutCrashB_snow");
		...
		if (self.firstCrash) then
		  ...
		  g_gameRules:ClientViewShake(pos,nil,0,30,1.5,0.25,0.05);
	  ...
	  if (not self.firstCrash) then				  
			...
			self.firstCrash = 1;			
			g_gameRules:ClientViewShake(pos,nil,0,90,3,0.5,0.05);			

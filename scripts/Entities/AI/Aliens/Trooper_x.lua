	...
	Vulnerability = 
  {
	...
	gameParams =
	{
		...
		autoAimTargetParams =
		{
			...
			outerRadius = 1.3,
			snapRadius = 0.14,
			snapRadiusTagged = 0.14,
		},
		
		boneIDs = 
		{
			BONE_BIP01 = "chest",
			BONE_SPINE = "Bip01 Neck",
			BONE_SPINE2 = "Bip01 Neck",
			BONE_SPINE3 = "Bip01 Neck",
			...
			BONE_FOOT_R = "chest",
			BONE_FOOT_L = "chest",
			BONE_ARM_R = "R_forearm",
			BONE_ARM_L = "L_forearm",
			BONE_CALF_R = "chest",
			BONE_CALF_L = "chest"
		},
...
--    self:SetAttachmentEffect(0, "damage_effect_1", "alien_special.Trooper.WoundedPlasma_death", g_Vectors.v000, g_Vectors.v010, 1, 0);
	...
	if (ragdoll) then
		...
		if (self.Properties.bCanSelfDestruct == 1) then
...
function Trooper_x.AutoDestruct(entity)--,timerid)
	...
	g_gameRules:ClientViewShake(pos,nil,0,10,2,0.25,0.0075);
	CopyVector(g_SignalData.point,entity:GetWorldPos());
...
function Trooper_x:SetGroupFireModes()
	...
	if (groupCount > 1) then
		...
		for k=1,groupCount do
			local member = AI.GetGroupMember(self.id,k,GROUP_ENABLED,AIOBJECT_ACTOR);
			if(member) then 
...
function Trooper_x:MeleeDamage(impulse,meleeType)
	...
	if (entity) then 	  
		...
		local bboxMin, bboxMax = self:GetWorldBBox();
		local targetBBoxMin, targetBBoxMax = entity:GetWorldBBox();
		local targetPos = { x = (targetBBoxMin.x + targetBBoxMax.x) * 0.5, y = (targetBBoxMin.y + targetBBoxMax.y) * 0.5, z = (targetBBoxMin.z + targetBBoxMax.z) * 0.5 }; 

		local rayPos = { x = (bboxMin.x + bboxMax.x) * 0.5, y = (bboxMin.y + bboxMax.y) * 0.5, z = (bboxMin.z + bboxMax.z) * 0.5 };
		local rayDir = { x = targetPos.x - rayPos.x, y = targetPos.y - rayPos.y, z = targetPos.z - rayPos.z };
		local rayHits = Physics.RayWorldIntersection(rayPos, rayDir, 1, ent_living+ent_independent, self.id, nil)
		
		local hitEnt = nil
		for rayHitIdx, rayHit in pairs(rayHits) do
			if rayHit.entity then
				hitEnt = rayHit.entity
			end
		end

		if hitEnt then
			local prevHitEnt = entity
			entity = hitEnt
	...
	--		  			end
							...
							hit.damage = (melee.damageSmall + (melee.damage - melee.damageSmall) * dot / 0.7) * self.melee.damageMultiplier ;								
						end
	...
	--					local distance = math.sqrt(distance2);
						...
						-- this angular shifting is very annoying on joypad
					...
					elseif (prevHitEnt.actor and prevHitEnt.actor:IsPlayer()) then
						prevHitEnt.actor:CameraShake(45, 0.3, 0.13, g_Vectors.v000);
...
function Trooper_x:AnimationEvent(event,value)
	...
	elseif ( event == "JumpMeleeGround" ) then
		...
		if(target) then 
			...
			if(angle) then 
				t = AI_G02.CanJumpToPoint(self.id,targetPos,angle,20,AI_JUMP_ON_GROUND + AI_JUMP_CHECK_COLLISION,vel);
			end
		end				

...
--	AI.ChangeParameter(self.id,AIPARAM_GROUPID,Properties.commrange);	-- communications range gets overriden when AI is killed
	...
	AI.ResetParameters(self.id, false, Properties, self.PropertiesInstance, nil,self.melee);
	AI.ChangeParameter(self.id,AIPARAM_COMBATCLASS,AICombatClasses.Infantry);
	AI.ChangeParameter(self.id,AIPARAM_FORGETTIME_TARGET,self.forgetTimeTarget);
	AI.ChangeParameter(self.id,AIPARAM_FORGETTIME_SEEK,self.forgetTimeSeek);
	AI.ChangeParameter(self.id,AIPARAM_FORGETTIME_MEMORY,self.forgetTimeMemory);
...
function BasicAI.Server:OnInitClient( channelId )
	if (self._enabled) then
		self.Client.ClAIEnable(self);
	else
		self.Client.ClAIDisable(self);
	end
end
...
function BasicAI:GetNearestInGroup()
	...
	if (groupCount > 1) then	
		local i = 1;
		local type = AI.GetTypeOf(self.id);
		while (i <= groupCount) do
			local member = AI.GetGroupMember( self.id, i, GROUP_ENABLED, type );
...
function BasicAI:SelectPrimaryWeapon()
	local primaryWeaponId=self.inventory:GetItemByClass(self.primaryWeapon);
	
	-- see if it is already selected
	local currentWeapon = self.inventory:GetCurrentItem();
	if(currentWeapon~=nil and currentWeapon.class==self.primaryWeapon) then return nil end
	self.actor:SelectItemByName(self.primaryWeapon);
	do return 1 end
end
...
function BasicAI:CheckReinforcements()
	AI.LogEvent(self:GetName().." >>> checking reinforcements, group count="..AI.GetGroupCount(self.id,GROUP_ENABLED,AIOBJECT_ACTOR));
	if(AI.GetGroupCount(self.id,GROUP_ENABLED,AIOBJECT_ACTOR)<3) then -- the 3rd is the one just died

...
function Hazard:OnPostLoad()
	-- this is used to put the entity in the correct state, the code side will already know this
	-- but the lua will potentially not be in the correct state internally, this forces that to correct itself
	self:GotoPostSerializedState();

	-- one time forcing of the timers so that any logic that needs to happen is done
...
Hazard.Server.Activated=
{
	OnBeginState = function(self)
		if(table.getn(self.ents)>0)then
			...
			for i,v in pairs(self.ents) do
				if(v.actor)then
					self.Client.AddScreenEffect(self, v.id);
				end;
			end;
		end;
	end,
	OnEnterArea = function(self,entity,areaId)
		...
		if(entity.actor)then
			self.Client.AddScreenEffect(self, entity.id);
		end;
	end,
	OnLeaveArea = function(self,entity,areaId)
		for i,v in pairs(self.ents) do
			if(v==entity)then
				...
				if(v.actor)then
					self.Client.RemoveScreenEffect(self, v.id);
				end;
				break;
			end;
		end;

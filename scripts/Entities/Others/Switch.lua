...
function Switch:OnUsed(user, idx)
		self.Server.SvRequestUse(self);
end;


----------------------------------------------------------------------------------------------------
-- clients use this to request a use to the server
function Switch.Server:SvRequestUse()
		self.Client.OnUsed_Internal(self);
end;


----------------------------------------------------------------------------------------------------
-- the function that really does the work when the switch is used
function Switch.Client:OnUsed_Internal()
...
-- (cant make GotoState() a client side callable function to just call it directly, because it is an internal C function. Or at least i dont know how)

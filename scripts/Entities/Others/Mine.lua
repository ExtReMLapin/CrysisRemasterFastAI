...
function Mine:Explode()
	...
	if(props.Claymore.bIsClaymore==0)then
		...
		self:PlaySoundEvent("sounds/physics:explosions:mine_explosion",{ x=0.0,y=0.0,z=0.0 },g_Vectors.v010,SOUND_DEFAULT_3D,SOUND_SEMANTIC_MECHANIC_ENTITY);
	else
		--Claymore
		...
		self:PlaySoundEvent("sounds/physics:explosions:claymore_explosion",{ x=0.0,y=0.0,z=0.0 },g_Vectors.v010,SOUND_DEFAULT_3D,SOUND_SEMANTIC_MECHANIC_ENTITY);
	end;
...
function Mine:Jump()
	...
	self:PlaySoundEvent("sounds/weapons:mine:mine_jump",{ x=0.0,y=0.0,z=0.0 },g_Vectors.v010,SOUND_DEFAULT_3D,SOUND_SEMANTIC_MECHANIC_ENTITY);
...
--				Log("Dist: "..dist.." Velocity: "..speed.." ("..0.55*(1+(dist/2.5)).." allowed)");
				if(speed>0.55*(1+(dist/2.5)))then
					local onGroundTime = (v.actorStats and v.actorStats.onGround) or 60.0;
					if(onGroundTime > 0.1)then
--						Log("Triggered by: "..v:GetName().." with weight: "..v:GetMass().." onGroundTime: "..onGroundTime);
						self:GotoState("Destroyed");
					end;
					break;
				end;

...
function SinglePlayer:EndLevel( params )
	if (not System.IsEditor()) then		  
		...
		g_GameTokenPreviousLevel = GameToken.GetToken( "Game.General.Previous_Level" );		
	end
end
...
----------------------------------------------------------------------------------------------------
function SinglePlayer:Filter(entity, collider, damage)
...
----------------------------------------------------------------------------------------------------
-- MOVED TO GameRules.cpp - Charlie 14/07/11
...
-- 	end
-- 
-- 	if (collider and collider == entity) then
-- 		return
-- 	end
-- 
-- 	local minVelocity;
...
-- 			-- IsPlayer
-- 			contactVelocitySq = vecLenSq(hit.target_velocity);
-- 			hit.velocity.x = 0.0;
-- 			hit.velocity.y = 0.0;
-- 			hit.velocity.z = 0.0;
-- 		else
...
-- 		if(contactMass > 200.0 and contactMass < 10000 and contactVelocitySq > 4) then
...
-- 					if(hit.target_velocity and vecLenSq(hit.target_velocity) < 10) then --limit damage from running agains static objects
-- 						damage = damage * 0.2;
-- 					end
-- 				end
-- 
...
-- 							entity.actor:Fall(hit.pos);
-- 						else  				  	
-- 							entity:Kill(true, collider.id, collider.id);  								
-- 						end
-- 					else
...
-- 				if(entity.actor and not entity.isAlien and not self:IsMultiplayer() and not AI.Hostile(entity.id, g_localActorId, false)) then
-- 					if(entity.id ~= g_localActorId) then
...
-- 					end
-- 				else
-- 					if(entity.actor and not entity.isAlien and collider and collider.actor and not self:IsMultiplayer()) then 
-- 						entity.actor:Fall(hit.pos);
-- 						return;
-- 					end
-- 				end
-- 				
-- 				-- Collision filtering for multiple hits
-- 				if (self:Filter(entity, collider, damage)) then
-- 					return;
-- 				end
-- 					
-- 				--if (debugColl>0) then
...
function SinglePlayer:OnCollisionEnd(currTime)
	local colHit = self.collisionHit;
	local entity = colHit.target;
	colHit.radius = 0;	
	colHit.partId = -1;
	colHit.targetId = colHit.target.id;
	colHit.weaponId = colHit.weapon.id;
	colHit.shooterId = colHit.shooter.id;
	colHit.materialId = 0;
	colHit.typeId = g_collisionHitTypeId;
	colHit.type = "collision";
	colHit.impulse=nil;

	colHit.target.lastCollDamagerId = colHit.weaponId;
	colHit.target.lastCollDamageTime = currTime;
	colHit.target.lastCollDamage = colHit.damage;
...
function SinglePlayer:OnCollisionShatter()
	local colHit = self.collisionHit;
	local entity = colHit.target;
	colHit.radius = 0;	
	colHit.partId = -1;
...
function SinglePlayer:ProcessActorDamage(hit)
	...
	if (health>0 and not target:IsOnVehicle() and target.AI) then
		if(target.AI.curSuitMode~=BasicAI.SuitMode.SUIT_ARMOR) then
		...
		target.actor:ActorAddImpulse(hit.partId, hit.dir);
	end
...
function SinglePlayer:ProcessDeath(hit)
	...
	if(hit.target.id == g_localActorId) then
		...
		if (not self:IsMultiplayer() and g_difficultyHintSystem>0 and g_difficultyLevel ~= 4 and
			...
			elseif (g_difficultyHintSystem == 2) then
				...
				if (self.lastSaveDeathCount>=g_difficultySaveThreshold) then
					hint = self.lastSaveDeathCount - g_difficultySaveThreshold;
					displayHint = true;
				end
			end
			...
			if (displayHint == true) then
				if (hint == 0) then
					HUD.DisplayBigOverlayFlashMessage("@HintArmor", 5.0, self.hudWhite);
				elseif (hint == 1) then
					HUD.DisplayBigOverlayFlashMessage("@HintStealth", 5.0, self.hudWhite);
				else
					if( g_difficultyLevel ~= 1 ) then
						HUD.DisplayBigOverlayFlashMessage("@HintDifficulty", 5.0, self.hudWhite);
					end
		...
		if(hit.shooterId == g_localActorId) then 
			local isFriendly = (not AI) or (not AI.Hostile(hit.target.id, g_localActorId, false))
			if (isFriendly) then -- should never happen atm, because we prevent death when player shot friend
			...
			self.game:SPNotifyPlayerKill(hit.target.id, hit.weaponId, isHeadShot, hit.projectileId or NULL_ENTITY, hit.typeId, isFriendly);
		end;
	end;
...
function SinglePlayer.Server:OnStartLevel()
	...
	if (g_GameTokenPreviousLevel) then
		GameToken.SetToken( "Game.General.Previous_Level", g_GameTokenPreviousLevel );
	end
end
...
function SinglePlayer:OnSave(table)
	table.gameTokenPreviousLevel = g_GameTokenPreviousLevel;
end


function SinglePlayer:OnLoad(table)
	g_GameTokenPreviousLevel = table.gameTokenPreviousLevel;
end
...
-- 		FastDifferenceVectors(vrel, hit.velocity, hit.target_velocity);
-- 		local dot = dotproduct3d(vrel, hit.normal);
-- 		dot = sqr(dot);
-- 		if (dot > 1000.0) then
-- 			dot = 0.0; -- Collision Problem !
-- 		end
-- 		v0Sq = dot;
-- 		v1Sq = dot;
-- 
...
-- 		
-- 	else	  
-- 		v0Sq = 2.0 * sqr(dotproduct3d(hit.velocity, hit.normal)); 
-- 		if (v0Sq > 2000.0) then
-- 			v0Sq = 0.0; -- Collision Problem !
-- 		end
-- 
